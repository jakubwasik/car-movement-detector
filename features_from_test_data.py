...
FILE = r"C:\Users\kuba\Desktop\praca magisterska\sensor data\training_data_1"
...
def features(filepath):
    ...
    for gps_file in glob.glob(os.path.join(filepath, "gps*")):
        print i
        event = re.search("_.*_", os.path.basename(gps_file)).group()[1:-1]
        date = re.search("2.*\.",  os.path.basename(gps_file)).group()[:-1]
        acc_file = glob.glob(os.path.join(filepath, "{event}_{date}*".format(**locals())))
...
if __name__ == '__main__':
    ...
    joblib.dump(clf, 'my_model_test_data.pkl', compress=9)
